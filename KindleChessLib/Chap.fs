...
module Chap =
    ...
    let ToVar i (gm0 : Game) =
        let ch = (i + 1).ToString()
        let nl = System.Environment.NewLine
        let gm = gm0|>Game.SetaMoves
        ...
        let mvltostr (mvl:MoveTextEntry list) =
            let mvtostr mv = 
                let str0 = mv|>PgnWrite.MoveTextEntryStr
            ...
            if mvl.IsEmpty then ""
                ...
                let fmvstr = mvl.Head|>PgnWrite.MoveTextEntryStr
                //let amv = 
                //    match fmv with
    ...
    let genh tfol ifol ofol i gm =
        let nl = System.Environment.NewLine
        let ch = gm|>Game.SetaMoves
        let isw = (gm.WhitePlayer<>"") && (gm.WhitePlayer<>"?")
        let nm = if isw then gm.WhitePlayer else gm.BlackPlayer
        ...
        let link bd dct id =
            ...
            let png = alias + ".png"
            // generate image
            let imgfol = Path.Combine(ofol, "board")
            do imgfol
               |> Directory.CreateDirectory
               |> ignore
            let flip = not isw
            do bd|>Png.BoardToPng ifol (Path.Combine(imgfol, png)) flip
        ...
        let rec dosubrav bd dct id (imtel:MoveTextEntry list) inmv ostr =
            if imtel.IsEmpty then (if inmv then ostr + "</em></p>" else ostr),dct
                ...
                |HalfMoveEntry(_,_,_,amv) ->
                    ...
                    if inmv then dosubrav nbd dct id imtel.Tail true (ostr + " " + (mte|>PgnWrite.MoveTextEntryStr))
                    else dosubrav nbd dct id imtel.Tail true (ostr + nl + "<p><em>" + (mte|>PgnWrite.MoveTextEntryStr))
                |NAGEntry(ng) ->
                    let nostr = if ostr.Length=0 then ostr else ostr.Substring(0,ostr.Length-1)
                    dosubrav bd dct id imtel.Tail inmv (nostr + (ng|>NagUtil.ToHtm))
                |CommentEntry(str) -> 
        ...
        let rec getmvs bd dct id (imtel:MoveTextEntry list) inmv ostr =
            if imtel.IsEmpty then (if inmv then ostr + "</strong></p>" else ostr),bd
                ...
                |HalfMoveEntry(_,_,_,amv) ->
                    let nbd = amv.Value.PostBrd
                    if inmv then getmvs nbd dct id imtel.Tail true (ostr + " " + (mte|>PgnWrite.MoveTextEntryStr))
                    else getmvs nbd dct id imtel.Tail true (ostr + nl + "<p><strong>" + (mte|>PgnWrite.MoveTextEntryStr))
                |NAGEntry(ng) ->
                    let nostr = if ostr.Length=0 then ostr else ostr.Substring(0,ostr.Length-1)
                    getmvs bd dct id imtel.Tail inmv (nostr + (ng|>NagUtil.ToHtm))
                |CommentEntry(str) -> 
                    let htm,ndct = str|>dod bd dct id
        ...
        let rec getravs bd id (iravl:MoveTextEntry list) ostr =
            ...
            let getravlnk i (rav:MoveTextEntry) =
                ...
                |RAVEntry(mtel) ->
                    ...
                    let mvstr = mv|>PgnWrite.MoveTextEntryStr
        ...
        let ouf = Path.Combine(ofol, "CH" + (i + 1).ToString() + ".html")
